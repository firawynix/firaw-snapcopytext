...
`SelectionGeometry.PlaceActionBar()` keeps the region action bar inside the
selection's lower-right corner with an inset. If the selection is smaller than
the bar, it maximizes overlap and clamps the bar to the virtual desktop so all
actions remain visible. `CaptureOverlayWindow.PositionActionBar()` recalculates
this after drawing, moving, or resizing.
...
Print Screen profile: `CapturePreferences` stores an independent enabled flag, normalized shortcut label, and `CaptureMode` for three slots. Defaults remain Print Screen → Region, Alt+Print Screen → Monitor, and Ctrl+Print Screen → Window for backward compatibility. `SettingsWindow` records Ctrl/Alt/Shift combinations with Print Screen, rejects duplicate enabled assignments, and lets each slot choose any capture mode. `HotkeyService.KeyboardProcedure()` matches the saved modifier combination and mode at runtime. `WH_KEYBOARD_LL` is primary so Windows reservations do not prevent capture; `RegisterHotKey` uses the saved modifiers as fallback. Only a matching enabled combination is suppressed; the replaced or disabled combination continues through `CallNextHookEx` for native Windows behavior. The custom shortcut remains a separate default-mode fallback.
...
Settings recording: `MainWindow.OpenSettings()` suspends current global hotkeys before showing the modal dialog and restores them in `finally`. `SettingsWindow.OnSourceInitialized()` temporarily registers all Ctrl/Alt/Shift modifier combinations of Print Screen while the dialog is open; its window hook records the one pressed in the focused profile field. `PreviewKeyUp` is a fallback for keyboards that expose Print Screen only on release.
